...
//
//  Created by Rodrigo Copetti on 21/08/2017.
//
//  Copyright © 2017 Rodrigo Copetti. All rights reserved.
//  Copyright © 2015 Elvis Nuñez
//  Copyright © 2016 SyncDB
//

import Foundation
import EncryptedCoreData

@objc public enum EncryptedDATAStackStoreType: Int {
    case inMemory, sqLite, sqLiteNoEncryption
    ...
    var type: String {
        ...
        case .sqLiteNoEncryption:
            return NSSQLiteStoreType
        }
    }
}

@objc public class EncryptedDATAStack: NSObject {
    private var storeType = EncryptedDATAStackStoreType.sqLite
    ...
    private var passphraseKey:String?
    ...
    public lazy var mainContext: NSManagedObjectContext = {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(EncryptedDATAStack.mainContextDidSave(_:)), name: .NSManagedObjectContextDidSave, object: context)
    ...
    public private(set) lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
        ...
        try! persistentStoreCoordinator.addPersistentStore(storeType: self.storeType, bundle: self.modelBundle, modelName: self.modelName, storeName: self.storeName, containerURL: self.containerURL, passphraseKey:self.passphraseKey)
    ...
    /**
     Initializes a EncryptedDATAStack using the bundle name as the model name, so if your target is called ModernApp,
    ...
    public convenience init(passphraseKey:String) {
        self.init()
        self.passphraseKey = passphraseKey
    }

    /**
     Initializes a EncryptedDATAStack using the provided model name.
     - parameter modelName: The name of your Core Data model (xcdatamodeld).
     */
    public init(passphraseKey:String, modelName: String) {
     ...
     */
    public init(passphraseKey:String? = nil, modelName: String, storeType: EncryptedDATAStackStoreType) {
        self.modelName = modelName
    ...
    /**
     ...
     based and doesn't save to disk, while the second one creates a .sqlite file and stores things there.
     */
    public init(passphraseKey:String? = nil, modelName: String, bundle: Bundle, storeType: EncryptedDATAStackStoreType) {
        self.modelName = modelName
    ...
    /**
     Initializes a EncryptedDATAStack using the provided model name, bundle, storeType and store name.
     - parameter modelName: The name of your Core Data model (xcdatamodeld).
     - parameter bundle: The bundle where your Core Data model is located, normally your Core Data model is in
     the main bundle but when using unit tests sometimes your Core Data model could be located where your tests
     are located.
     ...
     change that.
     */
    public init(passphraseKey:String? = nil, modelName: String, bundle: Bundle, storeType: EncryptedDATAStackStoreType, storeName: String) {
        self.modelName = modelName
        self.modelBundle = bundle
        self.storeType = storeType
     ...
     */
    public init(passphraseKey:String? = nil, modelName: String, bundle: Bundle, storeType: EncryptedDATAStackStoreType, storeName: String, containerURL: URL) {
        self.modelName = modelName
        self.modelBundle = bundle
        self.storeType = storeType
        self.storeName = storeName
    ...
    /**
     Initializes a EncryptedDATAStack using the provided model name, bundle and storeType.
     - parameter model: The model that we'll use to set up your EncryptedDATAStack.
     - parameter storeType: The store type to be used, you have .InMemory and .SQLite, the first one is memory
     based and doesn't save to disk, while the second one creates a .sqlite file and stores things there.
     */
    public init(passphraseKey:String? = nil, model: NSManagedObjectModel, storeType: EncryptedDATAStackStoreType) {
        self.model = model
        self.storeType = storeType
        self.passphraseKey = passphraseKey
    ...
    public func newDisposableMainContext() -> NSManagedObjectContext {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(EncryptedDATAStack.newDisposableMainContextWillSave(_:)), name: NSNotification.Name.NSManagedObjectContextWillSave, object: context)
    ...
    public func newBackgroundContext() -> NSManagedObjectContext {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(EncryptedDATAStack.backgroundContextDidSave(_:)), name: .NSManagedObjectContextDidSave, object: context)
    ...
    public func performInNewBackgroundContext(_ operation: @escaping (_ backgroundContext: NSManagedObjectContext) -> Void) {
        ...
        context.perform(EncryptedDATAStack.performSelectorForBackgroundContext(), with: blockObject)
    }
    ...
    func saveMainThread(completion: ((_ error: NSError?) -> Void)?) {
        ...
        let mainContextBlock: @convention(block) () -> Void = {
            self.writerContext.perform(EncryptedDATAStack.performSelectorForBackgroundContext(), with: writerContextBlockObject)
            DispatchQueue.main.async {
                completion?(writerContextError)
            }
        }
        let mainContextBlockObject: AnyObject = unsafeBitCast(mainContextBlock, to: AnyObject.self)
        self.mainContext.perform(EncryptedDATAStack.performSelectorForBackgroundContext(), with: mainContextBlockObject)
    }
                    ...
                    for store in self.persistentStoreCoordinator.persistentStores {
                        ...
                        do {
                            if #available(iOS 9.0, *) {
                                ...
                                try self.persistentStoreCoordinator.remove(self.persistentStoreCoordinator.persistentStores.last!)
                                try FileManager.default.removeItem(at: storeURL)
                            }
                            try! self.persistentStoreCoordinator.addPersistentStore(storeType: self.storeType, bundle: self.modelBundle, modelName: self.modelName, storeName: self.storeName, containerURL: self.containerURL,passphraseKey:self.passphraseKey)
    ...
    @objc func backgroundContextDidSave(_ notification: Notification) throws {
        if Thread.isMainThread && TestCheck.isTesting == false {
            ...
            let blockObject: AnyObject = unsafeBitCast(contextBlock, to: AnyObject.self)
            self.mainContext.perform(EncryptedDATAStack.performSelectorForBackgroundContext(), with: blockObject)
        }
    }
...
extension NSPersistentStoreCoordinator {
    func addPersistentStore(storeType: EncryptedDATAStackStoreType, bundle: Bundle, modelName: String, storeName: String?, containerURL: URL,passphraseKey:String? = nil) throws {
        ...
        case .inMemory:
            ...
            break
        case .sqLite, .sqLiteNoEncryption:
            if storeType == .sqLite{
                assert(passphraseKey != nil, "Set a Passphrase Key while using encrypted sqlite!")
            }
            ...
            if storeType == .sqLite {
                options[EncryptedStorePassphraseKey] = passphraseKey
            }
            do {
                try self.addPersistentStore(ofType: storeType.type, configurationName: nil, at: storeURL, options: options)
            } catch {
                do {
            ...
            let shouldExcludeSQLiteFromBackup = (storeType == .sqLite || storeType == .sqLiteNoEncryption) && TestCheck.isTesting == false
...
public extension NSManagedObjectModel {
    public convenience init(bundle: Bundle, name: String) {
...
extension NSError {
    convenience init(info: String, previousError: NSError?) {
        if let previousError = previousError {
            ...
            self.init(domain: "com.SyncDB.EncryptedDATAStack", code: 9999, userInfo: userInfo)
        }
    }
}
